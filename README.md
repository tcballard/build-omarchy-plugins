...
This checkout targets **v0.5.1**. Download published, checksummed assets from
[GitHub Releases](https://github.com/tcballard/build-omarchy-plugins/releases).
The commands below target v0.5.1; use them once that release is published.
A draft release or a newer `main` checkout is not a published release.

```bash
git clone --branch v0.5.1 --depth 1 https://github.com/tcballard/build-omarchy-plugins.git
...
## Claude Code plugin (v0.5.1)

Native Claude Code packaging was introduced in v0.3.1. From a reviewed v0.5.1 checkout,
...
avoid loading duplicate skills. The v0.5.1 release build includes a separate
`build-omarchy-plugins-claude-plugin-0.5.1.zip`, with the same twelve canonical
